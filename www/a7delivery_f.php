...
function ourview2($id,$divid,$zone,$phpAds_raw){
   ...
   if(!empty($phpAds_raw["bannerid"])){
	...
	$phpAds_raw["url"],
	$phpAds_raw["aRow"]["filename"],	$phpAds_raw["aRow"]["alt_filename"],
...
function ourview($inc_id,$divid,$zone,$bid, $cid, $url, $file,$file2, $type, $ver, $height,
...
?>
] , ['<?php echo $headers["X-Geo-Country-Code"]?>','<?php echo $headers["X-Geo-Region-Code"] ?>']);  <?php echo  $sErrors!=""?"Info.Warn('$sErrors');":"" ?>
